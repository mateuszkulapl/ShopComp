...
class CartController extends Controller
{
    public function index($eans='')
    {
        ...
        //for all shops, calculate the total price of all products in cart, if price is not available, get group avg price
        foreach ($shops as $shop) {
            $shop->totalPrice = $groups->pluck('products')->flatten()->where('shop_id', $shop->id)->sum(function ($product) {
                return $product->latestPrice->current ?? $product->group->avgPrice;
            });
        }
